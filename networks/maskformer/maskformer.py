...
class MaskFormer(nn.Module):
    ...
    def forward(self, x, encoder_only=False, skip_decoder: bool = False):
        ...
        # features: b x depth x n_dims x hw (vit) or b x n_dims x h x w (resnet50)
        ...
        if len(queries.shape) == 3:
            # b x n_queries x n_dims -> b x n_dims
            contrastive_features = queries.mean(dim=1)  # Global feature representation per image
        else:
            # b x n_layers x n_queries x n_dims -> b x n_dims
            contrastive_features = queries[:, -1].mean(dim=1)  # Use last layer features
        ...
        if self.arch == "vit_small":
            ...
            _h, _w = h, w
        upsampled_features: torch.Tensor = self.forward_pixel_decoder(
        ...
        if len(queries.shape) == 3:
            mask_pred = torch.einsum("bqn,bnhw->bqhw", queries, upsampled_features)
        else:
            if self.use_binary_classifier:
                mask_pred = torch.sigmoid(torch.einsum("bdqn,bnhw->bdqhw", queries, upsampled_features))
            else:
                mask_pred = torch.sigmoid(torch.einsum("bdqn,bnhw->bdqhw", self.ffn(queries), upsampled_features))
        ...
        if self.use_binary_classifier:
            ...
            batch_objectness = torch.sigmoid(objectness)
            dict_outputs.update({
                "objectness": batch_objectness,
                "mask_pred": mask_pred,
                "features": contrastive_features  # Always include features for contrastive learning
            })
        else:
            dict_outputs.update({
                "mask_pred": mask_pred,

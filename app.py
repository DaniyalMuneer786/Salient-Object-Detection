...
# Configure MySQL BEFORE creating the MySQL instance
app.config['MYSQL_HOST'] = "127.0.0.1"
app.config['MYSQL_USER'] = "root"
app.config['MYSQL_PASSWORD'] = ""  # Update this with your actual MySQL password if you have one
app.config['MYSQL_DB'] = "SOD"
app.config['MYSQL_DATETIME'] = True  # Ensure datetime objects are returned
app.config['MYSQL_CONNECT_TIMEOUT'] = 10  # Connection timeout in seconds
app.config['MYSQL_READ_TIMEOUT'] = 30     # Read timeout in seconds
app.config['MYSQL_WRITE_TIMEOUT'] = 30    # Write timeout in seconds

# Now initialize MySQL
try:
    mysql = MySQL(app)  # This must come AFTER the config
    logger.info("MySQL connection initialized successfully")
    
    # Note: Flask-MySQLdb uses lazy connections, so we'll test when actually needed
...
def initialize_database():
    ...
    try:
        if mysql is None:
            logger.error("Cannot initialize database: MySQL not available")
            return
            
        # Test connection first
        connection_ok, message = test_database_connection()
        if not connection_ok:
            logger.error(f"Database connection failed during initialization: {message}")
            raise Exception(f"Database connection failed: {message}")
            
        # Now create tables
        cur = mysql.connection.cursor()
        
        # Create user_management table if it doesn't exist
        cur.execute("""
...
class SelfMaskInference:
    ...
    def predict(self, image):
        ...
        try:
            ...
            # Generate heatmap (overlay mask on original)
            # Normalize mask for colormap
            mask_norm = (np.array(mask_img) / 255.0)
            cmap = plt.get_cmap('jet')
            heatmap_rgba = (cmap(mask_norm) * 255).astype(np.uint8)
            heatmap_img = Image.fromarray(heatmap_rgba).convert('RGBA').resize(original_img.size, Image.Resampling.LANCZOS)
            # Blend heatmap with original
            original_rgba = original_img.convert('RGBA')
            heatmap_blend = Image.blend(original_rgba, heatmap_img, alpha=0.5)
            # Optionally enhance for visibility
            heatmap_blend = ImageEnhance.Brightness(heatmap_blend).enhance(1.1)
            ...
            try:
                ...
                heatmap_blend.save(buffered_heatmap, format="PNG", quality=95)
...
# Initialize inference as None first
inference = None
...
try:
    ...
    if not os.path.exists(MODEL_PATH):
        ...
        logger.warning("AI model will not be available. Image processing will be limited.")
    ...
    logger.error(f"Traceback: {traceback.format_exc()}")
    logger.warning("AI model will not be available. Image processing will be limited.")
    # Don't raise the exception - let the app continue without AI model
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if 'user_id' not in session and 'admin_id' not in session:
            # Return JSON error instead of redirecting for API routes
            if request.path.startswith('/api/') or request.path in ['/save_image_info', '/predict', '/submit_feedback', '/get_feedback', '/get_payment_info']:
                return jsonify({
                    'success': False,
...
# ======================== Contact Page (alternative route) =============================
@app.route('/contact')
def contact_alt():
    return render_template('Contact.html')
...
def login():
    try:
        ...
        try:
            ...
            if "connection failed" in str(init_error).lower():
                return jsonify({
                    'success': False,
                    'message': 'Database connection failed. Please check if MySQL is running.'
                }), 500
            elif "access denied" in str(init_error).lower():
                return jsonify({
                    'success': False,
                    'message': 'Database access denied. Please check MySQL credentials.'
                }), 500
            else:
                return jsonify({
                    'success': False,
                    'message': f'Database error: {str(init_error)}'
                }), 500

        # Verify MySQL connection first
        connection_ok, message = test_database_connection()
        if not connection_ok:
            logger.error(f"Database connection failed during login: {message}")
            return jsonify({
                'success': False,
        ...
        try:
            ...
            if user and check_password_hash(user[2], input_password):
            ...
            logger.error(f"Database query error during login: {str(db_error)}")
            return jsonify({
                'success': False,
...
def save_image_info():
    try:
        ...
        # Database operations
        image_id = random.randint(1000, 20000)
        ...
        result_id = random.randint(2000, 30000)
...
def test_db():
    try:
        ...
        try:
            initialize_database()
            init_message = "Database tables initialized successfully"
        except Exception as init_error:
            init_message = f"Database initialization failed: {str(init_error)}"
            logger.warning(init_message)
        ...
        try:
            ...
            return jsonify({
                'success': False,
                'message': 'Database connection failed after initialization',
...
def test_connection():
    """Simple endpoint to test just the database connection"""
    try:
        if mysql is None:
            return jsonify({
                'success': False,

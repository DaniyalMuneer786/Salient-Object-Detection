...
# Initialize inference as None, will be set up on first use
inference = None

def initialize_inference():
    """Initialize the inference pipeline on first use"""
    global inference
...
# Helper decorator (no-op for demo/testing, like app1.py)
...
#         app.logger.info(f"Verifying OTP for email: {email}, OTP: {user_otp}")

#         otp_data = signup_otps.get(email)

#         if not otp_data:
#             app.logger.warning(f"No OTP data found for email: {email}")
#             return jsonify({'success': False, 'message': 'OTP expired or invalid. Please try signing up again.'}), 400

#         if datetime.now() > otp_data['expires']:
#             app.logger.warning(f"OTP expired for email: {email}")
#             del signup_otps[email]  # Clean up expired OTP
#             return jsonify({'success': False, 'message': 'OTP has expired. Please request a new one.'}), 400

#         if user_otp != otp_data['otp']:
#             app.logger.warning(f"Invalid OTP for {email}. Expected: {otp_data['otp']}, Got: {user_otp}")
...
#                            VALUES (%s, %s, %s, %s, %s)''',
#                         (otp_data['user_id'], 
#                          otp_data['user_data']['name'],
#                          otp_data['user_data']['email'], 
#                          otp_data['user_data']['password'],
#                          otp_data['user_data']['phone']))
#             mysql.connection.commit()
...
#     except Exception as e:
#         app.logger.error(f"❌ Error in verify_signup_otp: {str(e)}")
#         return jsonify({'success': False, 'message': 'Verification failed. Please try again.'}), 500
    
        
# def send_signup_otp_email(email, name, otp):
#     try:
#         msg = Message(
#             subject="🎉 Welcome to Salient Object Detection - Verify Your Email",
#             recipients=[email],
#             sender=app.config['MAIL_DEFAULT_SENDER']
#         )
...
#                     right: 0;
#                     height: 4px;
#                     background: linear-gradient(90deg, #00e6ac, #667eea, #764ba2, #00e6ac);
...
#                 <div class="content">
#                     <div class="greeting">Almost there! Let's verify your email</div>
#                     <div class="message">
#                         Thank you for registering with <strong>Salient Object Detection</strong>. 
...
#         mail.send(msg)
#         app.logger.info(f"Beautiful signup OTP sent to {email}")
#         return True
#     except Exception as e:
#         app.logger.error(f"Failed to send signup OTP email: {str(e)}")
#         return False
...
#         otp = str(secrets.randbelow(999999)).zfill(6)
#         expiration = datetime.now() + timedelta(minutes=10)
#         signup_otps[email]['otp'] = otp
#         signup_otps[email]['expires'] = expiration
        
#         # Send new OTP email
#         send_signup_otp_email(email, signup_otps[email]['user_data']['name'], otp)
...
#     try:
#         msg = Message(
#             subject="Welcome to Salient Object Detection",
...
#                 <p>Your account has been successfully created with our Salient Object Detection system.</p>
#                 <p>You can now login and start using our services.</p>
#                 <p>If you have any questions, please don't hesitate to contact our support team.</p>
#                 <p>Best regards,<br>The SOD Team</p><br><br>

#                 <p>Our Team Members:<br>==== 1. Daniyal Muneer ====<br>==== 2. Tayyab Riaz ====<br>==== 3. Noman Ali ====<br>==== 4. Zonia Tariq ====</p>
#                 <p>Thanks to Visit our SOD Website</p><br><br>
#                 <p>===================================================</p>
...
#         Your account has been successfully created with our Salient Object Detection system.
...
#         cur = mysql.connection.cursor()
#         cur.execute("""
#             SELECT Subscription_ID, Plan_Type, Start_Date, End_Date, Upload_Limit, Uploads_Used, Status
#             FROM subscription 
...
#         # Calculate remaining uploads
#         if subscription:
#             remaining_uploads = max(0, subscription[4] - (subscription[5] + 1))
#             message = f"Image processed successfully! {remaining_uploads} uploads remaining in your subscription."
#         else:
#             cur.execute("SELECT COUNT(*) FROM image WHERE user_id = %s", (user_id,))
#             upload_count = cur.fetchone()[0]
#             remaining_uploads = max(0, 3 - upload_count)
#             message = f"Image processed successfully! {remaining_uploads} free uploads remaining."
...
def save_image_info():
    try:
        ...
        if subscription:
            ...
            uploads_used = subscription[5] + 1
            remaining_uploads = "Unlimited" if upload_limit == -1 else max(0, upload_limit - uploads_used)
            message = f"Image processed successfully! Uploads remaining: {remaining_uploads}"
        else:
            cur.execute("SELECT COUNT(*) FROM image WHERE user_id = %s", (user_id,))
...
def feedback_stats():
    try:
        ...
        negative_words = ['bad', 'poor', 'hate', 'worst', 'terrible', 'disappointed', 'awful', 'issue']
...
def predict():
    try:
        ...
        # Use eager or lazy inference as already defined
